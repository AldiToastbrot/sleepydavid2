...
NGame = {
	...
	GAME_SPEED_SECONDS = { 600.0, 0.12, 0.23, 0.07, 0.0 }, -- game speeds for each level. Must be 5 entries with last one 0 for unbound
...
NCountry = {
	...
	REINFORCEMENT_MANPOWER_DELIVERY_SPEED = 100.0,	-- 100 Modifier for army manpower reinforcement delivery speed (travel time)
	REINFORCEMENT_MANPOWER_CHUNK = 0.5,				-- 0.1 Chunk size of manpower reinforcement delivery, in % of total manpower needed by the template.
	EQUIPMENT_UPGRADE_CHUNK_MAX_SIZE = 50,			-- 10 Maximum chunk size of equipment upgrade distribution per update.
	...
	SPECIAL_FORCES_CAP_BASE = 9999,					-- Max ammount of special forces battalions is total number of non-special forces battalions multiplied by this and modified by a country modifier
	...
	SCORCHED_EARTH_STATE_COST = 999,					-- pp cost to scorch a state
...
NResistance = {
	...
	RESISTANCE_TARGET_BASE = 25.0,							-- 25 base resistance target percentage

	RESISTANCE_TARGET_MODIFIER_HAS_CLAIM = -10.0,			-- 5 resistance target modifier in % for states we have claim
	...
	RESISTANCE_TARGET_MODIFIER_OCCUPIED_CAPITULATED = 5.0, -- 10 resistance target modifier when the enemy is capitulated

	RESISTANCE_TARGET_MODIFIER_OCCUPIED_IS_EXILE_MIN = 1.0,   -- 1 min & max resistance target modifier resistance target modifier for exile countries. interpolated using legitimacy
	RESISTANCE_TARGET_MODIFIER_OCCUPIED_IS_EXILE_MAX = 10.0,  -- 10
	...
	INITIAL_HISTORY_RESISTANCE = 0.0,			-- resistance value for initial colony state
...
NBuildings = {
	...
	NAVALBASE_REPAIR_MULT = 0.15,		-- Each level of navalbase building repairs X strength and can repair as many ships as its level
...
NMilitary = {
	COMBAT_VALUE_ORG_IMPORTANCE = 0,		-- 1 Multiplier on TotalOrganisation when determining the combat value of a division
	COMBAT_VALUE_STR_IMPORTANCE = 0,		-- 1 Multiplier on TotalStrength when determining the combat value of a division
	...
	LAND_COMBAT_ORG_DAMAGE_MODIFIER = 0.0575,       -- global damage modifier
	...
	UNIT_EXPERIENCE_PER_TRAINING_DAY = 0,
	...
	SLOWEST_SPEED = 4,
	REINFORCEMENT_REQUEST_MAX_WAITING_DAYS = 7,   -- #7 Every X days the equipment will be sent, regardless if still didn't produced all that has been requested.
	REINFORCEMENT_REQUEST_DAYS_FREQUENCY = 4,	   -- #4 How many days must pass until we may give another reinforcement request 
	...
	SUPPLY_USE_FACTOR_INACTIVE = 0.85,			   -- Deprecated/Unused
	...
	PROMOTE_LEADER_CP_COST = 1.0,					-- cost of promoting a leader
...
NNavy = {
	...
	TRAINING_EXPERIENCE_FACTOR = 0.6,								-- 0.3 Vanilla Amount of exp each ship gain every 24h while training (before modifiers)
...
NOperatives = {
	...
	AGENCY_CREATION_FACTORIES = 1,					-- Number of factories used to create an intelligence agency
